#!/bin/bash
# Script to run the benchmarks in Docker
...
docker-compose up -d scylla jupyter
...
CMD_ARGS="--data-sizes $DATA_SIZES --benchmark-types $BENCHMARK_TYPES"
...
docker-compose run --rm benchmark $CMD_ARGS
